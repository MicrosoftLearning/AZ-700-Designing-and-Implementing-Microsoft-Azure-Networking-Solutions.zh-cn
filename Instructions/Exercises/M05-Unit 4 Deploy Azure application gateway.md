...
1. 在 Cloud Shell 窗格的工具栏上，选择“**管理文件**”，然后选择“**上传**”。 上传以下文件：**backend.json**、**backend.parameters.json** 和 **install-iis.ps1**。
...
+ Azure 应用程序网关是一种 Web 流量（OSI 第 7 层）负载均衡器，便于管理流向 Web 应用程序的流量。
+ 应用程序网关可以根据 HTTP 请求的其他属性（例如 URI 路径或主机标头）进行路由决策。
