...
添加规则集合和规则以允许 Microsoft 域****

1. 选择“下一页:**** 规则”，然后选择“添加规则集合”****。
...
添加规则集合和规则，以允许远程桌面连接到 Srv-workload-01 虚拟机。****
...
1. 在“规则”**** 部分中。
   ...
   | 协议 | **TCP** |
   | 目标端口 | `3389` |
   | 目标（防火墙 IP 地址） | 输入防火墙虚拟中心公共 IP 地址 |
   | 已转换的类型 | **IP 地址** |
   | 已转换的地址或 FQDN | 输入 Srv-workload-01 虚拟机的专用 IP 地址 |
   | 已翻译的端口 | `3389` |
   
添加规则集合和规则，以允许远程桌面连接到 Srv-workload-02 虚拟机。****
...
1. 在“规则”**** 部分中。
   ...
   | 目标（防火墙 IP 地址） | 输入防火墙虚拟中心公共 IP 地址 |
   | 已转换的类型 | **IP 地址** |
   | 已转换的地址或 FQDN | 输入 Srv-workload-02 虚拟机的专用 IP 地址 |
   | 已翻译的端口 | `3389` |
